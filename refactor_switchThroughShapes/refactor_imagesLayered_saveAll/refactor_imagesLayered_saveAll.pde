...
- Make large enough images to print on cards
- Stop after n cards
- Save each image
- Make it fast (idea: through larger images alongside shorter 

Known issue: Images are old ones from BAC, not reachable! And wrong dimensions. 
             May need different dimension canvas for each image?
             
             May make sense to implement retrieving info from a csv or xls file or text file 
             - because only reading data!
             - if also writing, maybe need to database? Not just a text file
             
             MORE REFACTORING NECESSARY.
             
             Save as string dependent on time stamp. pretty please. 
*/
...
String[] imageStringArray = { // 18 imgs
