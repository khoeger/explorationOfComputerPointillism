...
int maxShapeHeight = maxShapeWidth;//+int(random(10, 20));
...
void drawShape() {
  ...
  if (a == 0) {                            // if over transparent background
    ...
    if (shapeType == "circle") {
      ...
      ellipse(0, 0, max(shapeWidth, shapeHeight), max(shapeWidth, shapeHeight));
      pop();
      pop();
      ...
      translate( x, y );      
      fill(r, g, b, alphaValue);
      rect(0, 0, max(shapeWidth, shapeHeight), max(shapeWidth, shapeHeight));
      pop();
      pop();
    }
